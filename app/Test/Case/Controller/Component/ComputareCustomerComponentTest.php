...
class ComputareCustomerComponentTest extends CakeTestCase {
	public $fixtures=array('app.customer','app.customerDetail','app.address','app.contact','app.customerGroup');
...
/*			'address1' => 'a1',
		...
		$this->assertTrue($this->ComputareCustomerComponent->save($newCust),'*initial save failed');
...
/*		$this->assertEquals('a1x',$record[0]['CustomerDetail']['address1'],'*Address1 save failure');
		...
		//test validation
		$record[0]['CustomerDetail']['firstName']=$record[0]['CustomerDetail']['lastName']=$record[0]['CustomerDetail']['companyName']='';
		$this->assertFalse($this->ComputareCustomerComponent->save($record[0]),'*Failed to catch empty names');
