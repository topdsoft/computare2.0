...
// debug($item);
		...
		if($this->data['Item']['step']==2) {
			...
			//if the item is allready on PO then use cost from PO, otherwise enter a cost
			if(isset($itemOnPO)) echo $this->Form->input('cost',array('disabled'=>true,'label'=>'Cost (Set on PO)'));
